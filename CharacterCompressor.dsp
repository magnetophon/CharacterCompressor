...
midStereoBlushComp(x,y) =(x+y)*ingain: ((( (crossfade(feedFwBw,_,_): ( COMP ))<:(_,_)) :((_,(x*ingain):gainHiShelfCrossfade(gainHS)),(_,(y*ingain):gainHiShelfCrossfade(gainHS))))~((_,_):+)):((_*(db2linear(outgain))),(_*(db2linear(outgain))));

midBlushComp(x,y) =(x+y)*ingain: (_ <:( crossfade(feedFwBw,_,_),_ : ( COMP , _ ) : gainHiShelfCrossfade(gainHS))~_)
*(db2linear(outgain))<:((_-(y:high_shelf(0,hiShelfFreq))),(_-(x:high_shelf(0,hiShelfFreq))));
